...
%
%	HShannon_kNN_k_estimation(Y,co) estimates the Shannon differential
% entropy (H) of Y using the kNN method (and neighbors S={k}).
% We use the naming convention 'H<name>_estimation' to ease embedding new
...
% Find squared distances
[d,num_of_samples] = size(Y);
squared_distances = kNN_squared_distances(Y,Y,co,1);

% co.mult:OK
% The information theoretical quantity of interest can be (and is!)
% estimated exactly [co.mult=1]; the computational complexity of the
% estimation is essentially the same as that of the 'up to multiplicative
% constant' case [co.mult=0]. In other words, the estimation is carried out
% 'exactly' (instead of up to 'proportionality').

% H estimation:
V = volume_of_the_unit_ball(d); 
H = log(num_of_samples-1) - psi(co.k) + log(V) + d / num_of_samples * sum(log(sqrt(squared_distances(co.k,:))));
