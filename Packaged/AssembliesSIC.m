...
for d = 1:N.datatype
	for c = 1:N.condition
		% 
		t = (T.index(2,:)==c);
		ts = timeseries.(datatype{d})(:,t);
		
		% Extract number of assemblies
		N.assemblies(d,c) = NumberofIC(ts);

		% Compute assembly activation timecourses and memberships
		[activations.(datatype{d}){1,c}, memberships.(datatype{d}){1,c}, W.(datatype{d})] = fastica(ts, 'numOfIC', N.assemblies(d,c), 'verbose','off');

		% Normalize memberships
		memberships.(datatype{d}){1,c} = memberships.(datatype{d}){1,c}./max(abs(memberships.(datatype{d}){1,c}));
		
		% Compute activation Shannon entropy and timecourse for each assembly
		[entro.(datatype{d}){1,c}, meanvalue.(datatype{d}){1,c}, activations.(datatype{d}){2,c}] = HShannon_segmented(activations.(datatype{d}){1,c}, T.index, N.condition, N.subjects, N.assemblies(d,c), co);
	end
end
clear d ts t
